...
/// @brief Generates a random number between -10 and +10, excluding 0.
/// @return Random number between -10 and 10, excluding 0.
int generateRandom() {
  int num;
  do {
    // Generate a random number between -10 and +10 (inclusive)
    num = rand() % 21 - 10;  // Use randomOffset
...
void loop() {
  ...
  printTime(timeinfo->tm_hour, (timeinfo->tm_min + randomOffset));

  if (lastUpdateMinute != timeinfo->tm_min) {
    // Generates a number between -10 and 10, excluding 0
    randomOffset = generateRandom();
